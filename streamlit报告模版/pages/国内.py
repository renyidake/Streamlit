...
dfm=df.loc[(df['标题'] != '-')]
dfm=dfm.loc[(dfm['受理局'] == '中国') | (dfm['受理局'] == '中国台湾') | (dfm['受理局'] == '中国澳门') | (dfm['受理局'] == '中国香港')]
st.dataframe(dfm)
# 指标 计算
...
def huitu21():
    ...
    def cunchupng() -> map:
        dfmb=dfm
        df1 = dfmb[['当前申请(专利权)人州/省', '公开(公告)号']]
        df1 = df1.rename(columns={'当前申请(专利权)人州/省': '当前申请人省'})
        series = df1['当前申请人省'].str.split('|', expand=True)  # 按照 | 分隔符拆分字段，用以清楚多余空格，对比以|拆分字段
        df_z = df1[['公开(公告)号']]
        df_11 = pd.DataFrame()
        for i in range(0, series.columns.size):
            df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
            df_l.columns = ['公开(公告)号', '当前申请人省']
            df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
        df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
        df_11 = pd.concat([df_11['公开(公告)号'], df_11['当前申请人省'].str.strip()],
                          axis=1)  # 用strip（）删除字符串头尾多余空格
        df_11 = df_11.groupby('当前申请人省', as_index=False)['公开(公告)号'].count()
        df_11 = df_11.sort_values(by='公开(公告)号', ascending=False)
        df_11.columns = ['当前申请人省', '申请数量']
        print(df_11)
        df1 = df_11
        df1.columns = ['当前申请专利权人省', '申请数量']
        df1 = df1.query('当前申请专利权人省 in %s ' % shengfen)
        df1.columns = ['当前申请人省', '申请数量']
        print(df1)
        listx2 = list(df1['当前申请人省'])
        listy2 = list(df1['申请数量'])
        data_pair2 = [list(z) for z in zip(listx2, listy2)]
        xmin = min(listy2)
        xmax = max(listy2)

        list1 = ['北京', '天津', '上海', '重庆']
        list2 = ['河北', '河南', '云南', '辽宁', '黑龙江', '湖南',
                 '安徽', '山东', '江苏', '浙江', '江西', '湖北', '甘肃', '山西',
                 '陕西', '吉林', '福建', '贵州', '广东', '青海', '四川',
                 '海南', '台湾', ]
        list3 = ['内蒙古', '西藏']
        list4 = ['香港', '澳门']
        list5 = ['新疆']
        list6 = ['宁夏']
        list7 = ['广西']
        for i in range(0, len(df1['当前申请人省'])):
            if df1.iat[i, 0] in list1:
                df1.iat[i, 0] = df1.iat[i, 0] + '市'
            if df1.iat[i, 0] in list2:
                df1.iat[i, 0] = df1.iat[i, 0] + '省'
            if df1.iat[i, 0] in list3:
                df1.iat[i, 0] = df1.iat[i, 0] + '自治区'
            if df1.iat[i, 0] in list4:
                df1.iat[i, 0] = df1.iat[i, 0] + '特别行政区'
            if df1.iat[i, 0] in list5:
                df1.iat[i, 0] = df1.iat[i, 0] + '维吾尔自治区'
            if df1.iat[i, 0] in list6:
                df1.iat[i, 0] = df1.iat[i, 0] + '回族自治区'
            if df1.iat[i, 0] in list7:
                df1.iat[i, 0] = df1.iat[i, 0] + '壮族自治区'
        print(df1)
        listx = list(df1['当前申请人省'])
        listy = list(df1['申请数量'])
        data_pair = [list(z) for z in zip(listx, listy)]
        xmin = min(listy)
        xmax = max(listy)
        map = (
            ...
            .add(series_name="", data_pair=data_pair, maptype="china",  # world，china 省 市
        ...
        #     .add("geo", data_pair, symbol_size=10, )  # 名字 数据 尺寸
        #     .set_series_opts(
        #         label_opts=opts.LabelOpts(
        #             is_show=True,
        #             position="inside",
...
def huitu22():
    ...
    def cunchupng() -> Line:
        ...
        dfmb=dfm
        dfmb = dfmb.rename(columns={'当前申请(专利权)人州/省': '当前申请人省'})

        df1 = dfmb[['当前申请人省', '公开(公告)号']]
        series = df1['当前申请人省'].str.split('|', expand=True)  # 按照 | 分隔符拆分字段，用以清楚多余空格，对比以|拆分字段
        df_z = df1[['公开(公告)号']]
        df_11 = pd.DataFrame()
        for i in range(0, series.columns.size):
        ...
        df_11 = df_11.query('当前申请人省 in %s ' % shengfen)
        dfx = df_11.head()
        print(dfx)
        ltx = list(dfx['当前申请人省'])
        print(ltx)

        df1 = dfmb[['公开(公告)号', '申请年', '当前申请人省', ]]
        df1 = df1.loc[(df1['当前申请人省'] == ltx[0])]
        df1 = df1[['公开(公告)号', '申请年']]
        df1 = df1.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df2 = dfmb[['公开(公告)号', '申请年', '当前申请人省', ]]
        df2 = df2.loc[(df2['当前申请人省'] == ltx[1])]
        df2 = df2[['公开(公告)号', '申请年']]
        df2 = df2.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df3 = dfmb[['公开(公告)号', '申请年', '当前申请人省', ]]
        df3 = df3.loc[(df3['当前申请人省'] == ltx[2])]
        df3 = df3[['公开(公告)号', '申请年']]
        df3 = df3.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df4 = dfmb[['公开(公告)号', '申请年', '当前申请人省', ]]
        df4 = df4.loc[(df4['当前申请人省'] == ltx[3])]
        df4 = df4[['公开(公告)号', '申请年']]
        df4 = df4.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df5 = dfmb[['公开(公告)号', '申请年', '当前申请人省', ]]
        df5 = df5.loc[(df5['当前申请人省'] == ltx[4])]
        df5 = df5[['公开(公告)号', '申请年']]
    ...
    # 使用st.components.v1.html显示HTML内容
    st.components.v1.html(html_with_css, height=500, scrolling=True)
st.subheader("""主要当前申请人省申请趋势""")
st.write('该图通过比较不同当前申请人省的折线，可以了解它们在专利申请数量方面的相对变化。高折线表示该当前申请人省的专利申请数量较多，而低折线表示申请数量较少。' \
         '图表中的高峰和低谷表示某些特定年份的专利申请数量。观察折线的走势可以了解专利申请数量的变化趋势。' \
         '通过比较不同当前申请人省的折线，可以观察到它们在排名方面的变化。折线交叉或靠近表示当前申请人省之间在排名上发生了变化。')
huitu22()
...
def huitu34():
    ...
    dfmb=dfm
    dfmb = dfmb.rename(columns={'当前申请(专利权)人州/省': '当前申请人省'})
    df1 = dfmb[['IPC分类号','当前申请人省', '公开(公告)号' ]]

    series = df1['IPC分类号'].str.split('|', expand=True)  # 按照 | 分隔符拆分字段，用以清楚多余空格，对比以|拆分字段
    df_z = df1[['当前申请人省', '公开(公告)号']]
    df_11 = pd.DataFrame()
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['当前申请人省', '公开(公告)号', 'IPC分类号']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['当前申请人省'],df_11['公开(公告)号'], df_11['IPC分类号'].str.strip()], axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)

    series = df_11['当前申请人省'].str.split('|', expand=True)  # 按照 | 分隔符拆分字段，用以清楚多余空格，对比以|拆分字段
    df_z = df_11[['公开(公告)号','IPC分类号']]
    df_11 = pd.DataFrame()
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['公开(公告)号','IPC分类号', '当前申请人省']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['公开(公告)号'],df_11['IPC分类号'], df_11['当前申请人省'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    df_11 = df_11.query('当前申请人省 in %s ' % shengfen)
    ...
    dfx = df_11.groupby('当前申请人省', as_index=False)['公开(公告)号'].count()
    dfx = dfx.sort_values(by='公开(公告)号', ascending=False)
    dfx.columns = ['当前申请人省', '申请数量']
    dfx = dfx.head()
    print(dfx)
    listx2 = list(dfx['当前申请人省'])
    print(listx2)
    ...
    df1 = df1.groupby(['当前申请人省', 'IPC分类号'], as_index=False).count()
    df1 = df1.sort_values(by='当前申请人省', ascending=True)
    df1.columns = ['当前申请人省', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('当前申请人省 in %s ' % listx2)
    df1.columns = ['当前申请人省', 'IPC分类号', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['IPC分类号'],df1['当前申请人省'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    # plt.ylabel('当前申请人省',fontdict={ 'size':14})
    # plt.xlabel('IPC分类号',fontdict={ 'size':14})
    plt.xticks(rotation=45,size=12)  # X轴刻度，标签，旋转度
    plt.yticks(size=12)
    for a, b, c in zip(df1['IPC分类号'],df1['当前申请人省'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)

st.subheader("""主要技术分支地域分布""")
st.write('该图表每个气泡的位置表示某个IPC分类和当前申请人省的组合，而气泡的大小可以表示该组合的相关度、重要性或其他指标。通过观察气泡的位置，可以判断某个IPC分类和当前申请人省之间的相关度。' \
         '气泡的大小可以表示IPC分类和当前申请人省的重要性或活跃度。较大的气泡表示该IPC分类和当前申请人省在申请数量或其他指标上具有较高的重要性或活跃度，而较小的气泡则表示重要性或活跃度较低。' \
         '通过观察气泡图在不同时间上的变化，可以了解IPC分类和当前申请人省的趋势。')
huitu34()
...
def huitu44():
    ...
    dfmb=dfm
    dfmb = dfmb.rename(columns={'当前申请(专利权)人州/省': '当前申请人省'})
    df1 = dfmb[['[标]当前申请(专利权)人','当前申请人省', '公开(公告)号' ]]
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['当前申请人省', '公开(公告)号', '[标]当前申请(专利权)人']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['当前申请人省'],df_11['公开(公告)号'], df_11['[标]当前申请(专利权)人'].str.strip()], axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['公开(公告)号', '[标]当前申请(专利权)人', '当前申请人省']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['公开(公告)号'], df_11['[标]当前申请(专利权)人'], df_11['当前申请人省'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    df_11 = df_11.query('当前申请人省 in %s ' % shengfen)
    ...
    dfx = df_11[['当前申请人省', '公开(公告)号']]
    dfx = dfx.groupby(['当前申请人省'], as_index=False)['公开(公告)号'].count()
    dfx = dfx.sort_values(by='公开(公告)号', ascending=False)
    dfx = dfx.head(10)
    print(dfx)
    listx2 = list(dfx['当前申请人省'])
    ...
    df1 = df1.groupby(['当前申请人省', '[标]当前申请(专利权)人'], as_index=False).count()
    df1 = df1.sort_values(by='当前申请人省', ascending=True)
    df1.columns = ['当前申请人省', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('当前申请人省 in %s ' % listx2)
    df1.columns = ['当前申请人省', '[标]当前申请(专利权)人', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['当前申请人省'],df1['[标]当前申请(专利权)人'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    # plt.ylabel('当前申请人省',fontdict={ 'size':14})
    # plt.xlabel('[标]当前申请(专利权)人',fontdict={ 'size':14})
    plt.xticks(rotation=45,size=10,ha='right')  # X轴刻度，标签，旋转度
    plt.tick_params(axis='x', pad=-5)
    plt.yticks(size=10)
    for a, b, c in zip(df1['当前申请人省'],df1['[标]当前申请(专利权)人'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)

st.subheader("""主要申请人地域分布""")
st.write('用于表示前十个当前申请人省和前十个申请人之间的关系和活跃度。通过观察气泡的位置，可以判断当前申请人省和申请人之间的关联程度。' \
         '气泡的大小可以表示当前申请人省和申请人的重要性或活跃度。较大的气泡表示该当前申请人省和申请人在专利申请数量或其他指标上具有较高的重要性或活跃度，而较小的气泡则表示重要性或活跃度较低。' \
         '通过比较不同当前申请人省和申请人的气泡大小，可以了解它们在专利申请方面的差异。')
huitu44()
...
def huitu54():
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['当前申请人省', '公开(公告)号', '发明人']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['当前申请人省'],df_11['公开(公告)号'], df_11['发明人'].str.strip()], axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['公开(公告)号', '发明人', '当前申请人省']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['公开(公告)号'], df_11['发明人'], df_11['当前申请人省'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    df_11 = df_11.query('当前申请人省 in %s ' % shengfen)
    ...
    df1 = df1.groupby(['当前申请人省', '发明人'], as_index=False).count()
    df1 = df1.sort_values(by='当前申请人省', ascending=True)
    df1.columns = ['当前申请人省', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('当前申请人省 in %s ' % listx2)
    df1.columns = ['当前申请人省', '发明人', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['当前申请人省'],df1['发明人'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    # plt.ylabel('当前申请人省',fontdict={ 'size':14})
    # plt.xlabel('发明人',fontdict={ 'size':14})
    plt.xticks(rotation=35,size=12)  # X轴刻度，标签，旋转度
    plt.yticks(size=12)
    for a, b, c in zip(df1['当前申请人省'],df1['发明人'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)
st.subheader("""主要发明人地域分布""")
st.write('通过观察气泡的位置，可以判断当前申请人省和发明人之间的关联程度。气泡的大小可以表示当前申请人省和发明人的重要性或活跃度。' \
         '较大的气泡表示该当前申请人省和发明人在专利申请数量或其他指标上具有较高的重要性或活跃度，而较小的气泡则表示重要性或活跃度较低。' \
         '通过比较不同当前申请人省和发明人的气泡大小，可以了解它们在专利申请方面的差异。')
huitu54()
