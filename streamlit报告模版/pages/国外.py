...
def huitu22():
    ...
    def cunchupng() -> Line:
        ...
        dfmb=dfm
        dfx = dfmb[['公开(公告)号', '申请年','受理局',]]
        dfx = dfx.groupby('受理局', as_index=False)['公开(公告)号'].count()
        ...
        df1 = dfmb[['公开(公告)号', '申请年', '受理局', ]]
        df1 = df1.astype({'申请年': 'str'})
        df1 = df1.query('申请年 in %s ' % nianfen)
        df1 = df1.loc[(df1['受理局'] == ltx[0])]
        df1 = df1[['公开(公告)号', '申请年']]
        df1 = df1.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df2 = dfmb[['公开(公告)号', '申请年', '受理局', ]]
        df2 = df2.astype({'申请年': 'str'})
        df2 = df2.query('申请年 in %s ' % nianfen)
        df2 = df2.loc[(df2['受理局'] == ltx[1])]
        df2 = df2[['公开(公告)号', '申请年']]
        df2 = df2.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df3 = dfmb[['公开(公告)号', '申请年', '受理局', ]]
        df3 = df3.astype({'申请年': 'str'})
        df3 = df3.query('申请年 in %s ' % nianfen)
        df3 = df3.loc[(df3['受理局'] == ltx[2])]
        df3 = df3[['公开(公告)号', '申请年']]
        df3 = df3.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df4 = dfmb[['公开(公告)号', '申请年', '受理局', ]]
        df4 = df4.astype({'申请年': 'str'})
        df4 = df4.query('申请年 in %s ' % nianfen)
        df4 = df4.loc[(df4['受理局'] == ltx[3])]
        df4 = df4[['公开(公告)号', '申请年']]
        df4 = df4.groupby('申请年', as_index=False)['公开(公告)号'].count()
...
def huitu23():
    ...
    df1 = df1[['公开(公告)号', '受理局', '优先权国家']]
    series = df1['优先权国家'].str.split('|', expand=True)  # 按照 分隔符拆分字段
    df_z = df1[['公开(公告)号', '受理局']]
    df_11 = pd.DataFrame()
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['公开(公告)号', '受理局', '优先权国家']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['公开(公告)号'],df_11['受理局'], df_11['优先权国家'].str.strip()], axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    df_11 = df_11.groupby(['受理局', '优先权国家'], as_index=False)['公开(公告)号'].count()
...
st.subheader("""五局流向图""")
st.write('该图表展示了近20年专利运营情况，包括权力转移、质押和许可。通过观察圆形的大小可以比较不同运营方式在不同年份中的专利数量变化。如果某个运营方式的圆形在连续几年中保持较大的大小，' \
         '表示对应的运营方式在这段时间内具有较高的专利数量。相反，表示该运营方式的专利数量有较大的波动。该图表可以用于分析专利的运营方式及其变化趋势，帮助了解专利的商业利用和价值实现方式。' \
         '通过观察不同运营方式的圆形大小，可以了解到各个运营方式的相对重要性和在不同年份的演变情况。这有助于评估专利的商业价值和知识产权的管理策略。')
huitu23()
...
def huitu34():
    global document
    dfmb=dfm
    df1 = dfmb[['IPC分类号','受理局', '公开(公告)号' ]]

    series = df1['IPC分类号'].str.split('|', expand=True)  # 按照 | 分隔符拆分字段，用以清楚多余空格，对比以|拆分字段
    df_z = df1[['受理局', '公开(公告)号']]
    df_11 = pd.DataFrame()
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['受理局', '公开(公告)号', 'IPC分类号']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['受理局'],df_11['公开(公告)号'], df_11['IPC分类号'].str.strip()], axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    dfx = df_11[['IPC分类号', '公开(公告)号']]
    dfx = dfx.groupby(['IPC分类号'], as_index=False)['公开(公告)号'].count()
    dfx = dfx.sort_values(by='公开(公告)号', ascending=False)
    ...
    df1 = df1.groupby(['受理局', 'IPC分类号'], as_index=False).count()
    df1 = df1.sort_values(by='受理局', ascending=True)
    df1.columns = ['受理局', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('受理局 in %s ' % listx2)
    df1.columns = ['受理局', 'IPC分类号', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['IPC分类号'],df1['受理局'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    # plt.ylabel('受理局',fontdict={ 'size':14})
    # plt.xlabel('IPC分类号',fontdict={ 'size':14})
    plt.xticks(rotation=45,size=12)  # X轴刻度，标签，旋转度
    plt.yticks(size=12)
    for a, b, c in zip(df1['IPC分类号'],df1['受理局'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)
...
def huitu44():
    global document
    dfmb=dfm
    df1 = dfmb[['[标]当前申请(专利权)人','受理局', '公开(公告)号' ]]
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['受理局', '公开(公告)号', '[标]当前申请(专利权)人']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['受理局'],df_11['公开(公告)号'], df_11['[标]当前申请(专利权)人'].str.strip()], axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    dfx = df_11[['[标]当前申请(专利权)人', '公开(公告)号']]
    dfx = dfx.groupby(['[标]当前申请(专利权)人'], as_index=False)['公开(公告)号'].count()
    dfx = dfx.sort_values(by='公开(公告)号', ascending=False)
    ...
    df1 = df1.groupby(['受理局', '[标]当前申请(专利权)人'], as_index=False).count()
    df1 = df1.sort_values(by='受理局', ascending=True)
    df1.columns = ['受理局', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('受理局 in %s ' % listx2)
    df1.columns = ['受理局', '[标]当前申请(专利权)人', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['受理局'],df1['[标]当前申请(专利权)人'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    # plt.ylabel('受理局',fontdict={ 'size':14})
    # plt.xlabel('[标]当前申请(专利权)人',fontdict={ 'size':14})
    plt.xticks(rotation=45,size=10,ha='right')  # X轴刻度，标签，旋转度
    plt.tick_params(axis='x', pad=-5)
    plt.yticks(size=10)
    for a, b, c in zip(df1['受理局'],df1['[标]当前申请(专利权)人'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)
...
def huitu54():
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['受理局', '公开(公告)号', '发明人']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['受理局'],df_11['公开(公告)号'], df_11['发明人'].str.strip()], axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    df1 = df1.groupby(['受理局', '发明人'], as_index=False).count()
    df1 = df1.sort_values(by='受理局', ascending=True)
    df1.columns = ['受理局', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('受理局 in %s ' % listx2)
    df1.columns = ['受理局', '发明人', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['受理局'],df1['发明人'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    # plt.ylabel('受理局',fontdict={ 'size':14})
    # plt.xlabel('发明人',fontdict={ 'size':14})
    plt.xticks(rotation=35,size=12)  # X轴刻度，标签，旋转度
    plt.yticks(size=12)
    for a, b, c in zip(df1['受理局'],df1['发明人'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)
