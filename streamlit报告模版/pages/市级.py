...
dfm=df.loc[(df['标题'] != '-')]
dfm=dfm.loc[dfm['当前申请(专利权)人地市'].str.contains(dishi,na=False),:]
st.dataframe(dfm)
# 指标 计算
...
def huitu21():
    ...
    def cunchupng() -> map:
        dfmb=dfm
        dfmb = dfmb.rename(columns={'当前申请(专利权)人区县': '当前申请人区县'})
        df1 = dfmb[['当前申请人区县', '公开(公告)号']]
        series = df1['当前申请人区县'].str.split('|', expand=True)  # 按照 | 分隔符拆分字段，用以清楚多余空格，对比以|拆分字段
        df_z = df1[['公开(公告)号']]
        df_11 = pd.DataFrame()
        for i in range(0, series.columns.size):
            df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
            df_l.columns = ['公开(公告)号', '当前申请人区县']
            df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
        df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
        df_11 = pd.concat([df_11['公开(公告)号'], df_11['当前申请人区县'].str.strip()],
                          axis=1)  # 用strip（）删除字符串头尾多余空格
        df_11 = df_11.groupby('当前申请人区县', as_index=False)['公开(公告)号'].count()
        df_11 = df_11.sort_values(by='公开(公告)号', ascending=False)
        df_11.columns = ['当前申请人区县', '申请数量']

        df1 = df_11
        listx = list(df1['当前申请人区县'])
        listy = list(df1['申请数量'])
        data_pair = [list(z) for z in zip(listx, listy)]
        xmin = min(listy)
        xmax = max(listy)
        map = (
            ...
            .add(series_name="", data_pair=data_pair, maptype=dishi,  # world，china 省 市
...
def huitu22():
    ...
    def cunchupng() -> Line:
        ...
        dfx = df_11.head()
        print(dfx)
        ltx = list(dfx['当前申请人区县'])
        print(ltx)

        df1 = dfmb[['公开(公告)号', '申请年', '当前申请人区县', ]]
        df1 = df1.loc[(df1['当前申请人区县'] == ltx[0])]
        df1 = df1[['公开(公告)号', '申请年']]
        df1 = df1.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df2 = dfmb[['公开(公告)号', '申请年', '当前申请人区县', ]]
        df2 = df2.loc[(df2['当前申请人区县'] == ltx[1])]
        df2 = df2[['公开(公告)号', '申请年']]
        df2 = df2.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df3 = dfmb[['公开(公告)号', '申请年', '当前申请人区县', ]]
        df3 = df3.loc[(df3['当前申请人区县'] == ltx[2])]
        df3 = df3[['公开(公告)号', '申请年']]
        df3 = df3.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df4 = dfmb[['公开(公告)号', '申请年', '当前申请人区县', ]]
        df4 = df4.loc[(df4['当前申请人区县'] == ltx[3])]
        df4 = df4[['公开(公告)号', '申请年']]
        df4 = df4.groupby('申请年', as_index=False)['公开(公告)号'].count()
        ...
        df5 = dfmb[['公开(公告)号', '申请年', '当前申请人区县', ]]
        df5 = df5.loc[(df5['当前申请人区县'] == ltx[4])]
        df5 = df5[['公开(公告)号', '申请年']]
...
def huitu34():
    ...
    dfmb = dfm
    dfmb = dfmb.rename(columns={'当前申请(专利权)人区县': '当前申请人区县'})
    df1 = dfmb[['IPC分类号', '当前申请人区县', '公开(公告)号']]

    series = df1['IPC分类号'].str.split('|', expand=True)  # 按照 | 分隔符拆分字段，用以清楚多余空格，对比以|拆分字段
    df_z = df1[['当前申请人区县', '公开(公告)号']]
    df_11 = pd.DataFrame()
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['当前申请人区县', '公开(公告)号', 'IPC分类号']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['当前申请人区县'], df_11['公开(公告)号'], df_11['IPC分类号'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)

    series = df_11['当前申请人区县'].str.split('|', expand=True)  # 按照 | 分隔符拆分字段，用以清楚多余空格，对比以|拆分字段
    df_z = df_11[['公开(公告)号', 'IPC分类号']]
    df_11 = pd.DataFrame()
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['公开(公告)号', 'IPC分类号', '当前申请人区县']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['公开(公告)号'], df_11['IPC分类号'], df_11['当前申请人区县'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    dfx = df_11.groupby('当前申请人区县', as_index=False)['公开(公告)号'].count()
    dfx = dfx.sort_values(by='公开(公告)号', ascending=False)
    dfx.columns = ['当前申请人区县', '申请数量']
    dfx = dfx.head()
    print(dfx)
    listx2 = list(dfx['当前申请人区县'])
    print(listx2)

    df1 = df_11

    df1 = df1.groupby(['当前申请人区县', 'IPC分类号'], as_index=False).count()
    df1 = df1.sort_values(by='当前申请人区县', ascending=True)
    df1.columns = ['当前申请人区县', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('当前申请人区县 in %s ' % listx2)
    df1.columns = ['当前申请人区县', 'IPC分类号', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['IPC分类号'],df1['当前申请人区县'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    # plt.ylabel('当前申请人区县',fontdict={ 'size':14})
    # plt.xlabel('IPC分类号',fontdict={ 'size':14})
    plt.xticks(rotation=45,size=12)  # X轴刻度，标签，旋转度
    plt.yticks(size=12)
    for a, b, c in zip(df1['IPC分类号'],df1['当前申请人区县'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)
...
def huitu44():
    ...
    dfmb = dfm
    dfmb = dfmb.rename(columns={'当前申请(专利权)人区县': '当前申请人区县'})
    df1 = dfmb[['[标]当前申请(专利权)人', '当前申请人区县', '公开(公告)号']]
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['当前申请人区县', '公开(公告)号', '[标]当前申请(专利权)人']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['当前申请人区县'], df_11['公开(公告)号'], df_11['[标]当前申请(专利权)人'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['公开(公告)号', '[标]当前申请(专利权)人', '当前申请人区县']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['公开(公告)号'], df_11['[标]当前申请(专利权)人'], df_11['当前申请人区县'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    dfx = df_11[['当前申请人区县', '公开(公告)号']]
    dfx = dfx.groupby(['当前申请人区县'], as_index=False)['公开(公告)号'].count()
    dfx = dfx.sort_values(by='公开(公告)号', ascending=False)
    dfx = dfx.head(10)
    print(dfx)
    listx2 = list(dfx['当前申请人区县'])
    ...
    df1 = df1.groupby(['当前申请人区县', '[标]当前申请(专利权)人'], as_index=False).count()
    df1 = df1.sort_values(by='当前申请人区县', ascending=True)
    df1.columns = ['当前申请人区县', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('当前申请人区县 in %s ' % listx2)
    df1.columns = ['当前申请人区县', '[标]当前申请(专利权)人', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['当前申请人区县'],df1['[标]当前申请(专利权)人'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    ...
    plt.yticks(size=10)
    for a, b, c in zip(df1['当前申请人区县'],df1['[标]当前申请(专利权)人'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)
...
def huitu54():
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['当前申请人区县', '公开(公告)号', '发明人']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['当前申请人区县'], df_11['公开(公告)号'], df_11['发明人'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    for i in range(0, series.columns.size):
        df_l = pd.concat([df_z, series[i]], axis=1)  ##公开号与拆分后的一列申请人数据结合成新表
        df_l.columns = ['公开(公告)号', '发明人', '当前申请人区县']
        df_11 = pd.concat([df_11, df_l])  ##所有新表叠加
    df_11.dropna(inplace=True)  # 删除空数据，获得有效数据
    df_11 = pd.concat([df_11['公开(公告)号'], df_11['发明人'], df_11['当前申请人区县'].str.strip()],
                      axis=1)  # 用strip（）删除字符串头尾多余空格
    print(df_11)
    ...
    df1 = df1.groupby(['当前申请人区县', '发明人'], as_index=False).count()
    df1 = df1.sort_values(by='当前申请人区县', ascending=True)
    df1.columns = ['当前申请人区县', 'IPC主分类号小类', '申请数量']
    print(df1)
    df1 = df1.query('IPC主分类号小类 in %s ' % listx)
    df1 = df1.query('当前申请人区县 in %s ' % listx2)
    df1.columns = ['当前申请人区县', '发明人', '申请数量']
    print(df1)
    ...
    plt.grid(ls='-.', lw=0.35)  # 增加栅格
    plt.scatter( df1['当前申请人区县'],df1['发明人'], df1['申请数量']/xmax*1000,c=color, alpha=0.7)
    # plt.ylabel('受理局',fontdict={ 'size':14})
    # plt.xlabel('发明人',fontdict={ 'size':14})
    plt.xticks(rotation=35,size=12)  # X轴刻度，标签，旋转度
    plt.yticks(size=12)
    for a, b, c in zip(df1['当前申请人区县'],df1['发明人'],  df1['申请数量']):
        plt.text(a, b, c, ha='center', va='center', fontsize=8, alpha=0.9)
    st.pyplot(fig)
